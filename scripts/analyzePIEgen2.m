% FILENAME: analyzePIEgen2.m, First Created on Jan. 04, 2012
%
% REVISION HISTORY:
%
...
PARAMS.INCLUDE_SCATTERING = 1; % this shifts the time gate back to include the whole TCSPC histogram
PARAMS.BIN_TIME = 1.0; % in millisecond
...
PARAMS.GAMMA = 1.0; 
PARAMS.NOT_PIE = 3; % this is very tight. Used to identify a donor-only species. Want to make sure that acceptor is definately not there!
...
if not(length(DESC))
    ...
    PREFS.DESC_LABEL = sprintf('(%s) ',DESC);
end;
%
% display(inFile)
fprintf(1,'---------------------------------------------------------\n');
...
% This idea didn't really pan out. Forget about trying to get rid of scattered light.
%
...
% >> correct for possible differences in bin size
...
redFilter = ((AA > PARAMS.PIE_MIN) & (AA < PARAMS.PIE_MAX)); % PIE filter
notRedFilter = (AA < PARAMS.NOT_PIE) & (DA < PARAMS.NOT_PIE); % identifies sample c/ no red counts
greenFilter = ((DD+DA > PARAMS.NOISE_MIN) & (DD+DA < PARAMS.NOISE_MAX)); % burst selection
